...
import { createCopy, createTag, readBlockConfig, toCamelCase } from "./dom.js";
...
export function convertBlockToTable(block, name, path) {
  ...
  headerRow.append(
    createTag(
      ...
      {
        colspan: maxCols,
        style: `background-color: ${getPreferedBackgroundColor()}; color: ${getPreferedForegroundColor()};`,
      },
      name
    )
  );
...
function getSectionMetadata(block, baseURL) {
  ...
  if (sectionMetadata) {
    ...
    return convertBlockToTable(sectionMetadata, "Section metadata", baseURL);
  }
}
...
export function copyBlockToClipboard(wrapper, name, blockURL) {
  ...
  if (element) {
    blockTable = convertBlockToTable(element, name, blockURL);
  }
